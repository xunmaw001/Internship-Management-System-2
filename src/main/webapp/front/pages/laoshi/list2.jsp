...
<!-- 标题 -->
 ...
 <div class="proleft fl" style="width: 20%;">
     ...
     <ul class="ul1">
         <li v-for="(item,index) in centerMenu" v-bind:key="index" :class="item.url=='../laoshi/list.jsp'?'on':''" @click="jump(item.url)">
             <b></b>{{item.name}}
         </li>
     </ul>
 </div>        <!-- 个人中心菜单 -->
...
<!--                <button @click="jump('../laoshi/add.jsp')" class="layui-btn layui-btn-lg btn-theme sub_backgroundColor" :style='{"padding":"0 15px","boxShadow":"0 0 8px rgba(0,0,0,0)","margin":"0 0 0 10px","borderColor":"#409EFF","color":"#333","borderRadius":"4px","borderWidth":"0","width":"auto","fontSize":"14px","borderStyle":"solid","height":"40px"}'>
                    <i v-if="true" class="layui-icon">&#xe654;</i> 添加任课老师信息
                </button>-->
            </div>
            <table class="layui-table" lay-skin="nob">
                ...
                <tbody>
                    <tr v-for="(item,index) in dataList" v-bind:key="index">
                        <td>{{item.username}}</td>
                        <td>{{item.laoshiName}}</td>
                        <td>{{item.laoshiPhone}}</td>
                        <td>{{item.laoshiIdNumber}}</td>
                        <td><img :src="baseUrl+item.laoshiPhoto" style="width: 100px;height: 100px;object-fit: cover;"></td>
                        <td>{{item.sexValue}}</td>
                        <td>{{item.laoshiEmail}}</td>
                        <td style="width: 100px;">
                        <!--
                            <button @click="jump('../laoshi/add.jsp?laoshiId='+item.id)" type="button" class="layui-btn layui-btn-sm layui-btn-radius layui-btn-warm">
...
<script type="text/javascript">
    ...
    var vue = new Vue({
        ...
        methods: {
            ...
            deleteData(data){
                ...
                // 删除信息
                layui.http.requestJson(`laoshi/delete`, 'post', [data.id], (res) => {
                    if(res.code==0){
                        layer.msg('删除成功', {
                            time: 2000,
                            icon: 6
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage', 'http', 'jquery', 'laydate', 'tinymce'], function() {
        ...
        localStorage.setItem("goUtl","./pages/laoshi/list2.jsp")
        ...
        // var id = http.getParam('id');
           ...
           function sexTypesSelect() {
            ...
            function pageList() {
                // 获取列表数据
                http.request('laoshi/list', 'get', vue.searchForm, function (res) {
                    ...
                    laypage.render({
                        ...
                        jump: function (obj, first) {
                            ...
                            //首次不执行
                            if (!first) {
                                http.request('laoshi/list', 'get', vue.searchForm, function (res1) {
