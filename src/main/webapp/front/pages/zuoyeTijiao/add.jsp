    ...
    <div class="data-add-container  sub_borderColor"
        ...
        <form class="layui-form layui-form-pane add" lay-filter="myForm">
            ...
            <div class="layui-form-item main_borderColor"
                ...
                <div class="layui-input-block">
                    <select name="laoshiId" id="laoshiId" lay-filter="laoshiId">
                        ...
                        <option v-for="(item,index) in laoshiList" v-bind:key="index" :value="item.id"
                                :selected="detail.laoshiId == item.id?true:false" :key="item.id">{{ item.laoshiName }}
                        </option>
                    </select>
                </div>
            </div>
            ...
            <div class="layui-form-item main_borderColor"
                ...
                <div class="layui-input-block">
                    <select name="zuoyeId" id="zuoyeId" lay-filter="zuoyeId">
                        <option value="">请选择</option>
                        <option v-for="(item,index) in zuoyeList" v-bind:key="index" :value="item.id"
                                :selected="detail.zuoyeId == item.id?true:false" :key="item.id">{{ item.zuoyeName }}
                        </option>
                    </select>
                </div>
            </div>
            ...
            <div class="layui-form-item main_borderColor"
                ...
                <div class="layui-input-block">
                    <input class="layui-input main_borderColor"
                           ...
                           v-model="detail.zuoyeTijiaoUuidNumber" lay-verify="required" type="text"
                           :readonly="ro.zuoyeTijiaoUuidNumber" name="zuoyeTijiaoUuidNumber" id="zuoyeTijiaoUuidNumber"
                           autocomplete="off">
                </div>
            </div>
            <!-- 级联表的表id -->
<%--            <input type="hidden" :value="detail.zuoyeId" id="zuoyeId" name="zuoyeId"/>--%>
            <!-- 级联表的表id -->
            ...
            <div class="layui-form-item main_borderColor"
                ...
                <div class="layui-input-block">
                    <textarea class="main_borderColor layui-input"
                              ...
                              v-model="detail.zuoyeTijiaoText" lay-verify="required" :readonly="ro.zuoyeTijiaoText"
                              name="zuoyeTijiaoText" id="zuoyeTijiaoText" autocomplete="off"></textarea>
                </div>
            </div>
            <div class="layui-form-item main_borderColor"
                 :style='{"padding":"10px","boxShadow":"0 0 0px rgba(255,0,0,.8)","margin":"0 0 10px 0","backgroundColor":"rgba(255, 255, 255, 0)","borderRadius":"1px","borderWidth":"0 0 1px 0","borderStyle":"solid"}'>
                ...
                <div class="layui-input-block">
                    <div v-if="detail.zuoyeShangchuanFile" style="display:inline-block;margin-right:10px;">
                        <button v-if="detail.zuoyeShangchuanFile != null"
                                @click="jump(baseUrl+detail.zuoyeShangchuanFile)" id="zuoyeShangchuanFileImg"
                                type="button" class="layui-btn layui-btn-success">点击查看
                        </button>
                        <span v-if="detail.zuoyeShangchuanFile == null">暂无文件</span>
                        <input type="hidden" :value="detail.zuoyeShangchuanFile" id="zuoyeShangchuanFile"
                               name="zuoyeShangchuanFile"/>
                    </div>
                    <button v-if="!ro.zuoyeShangchuanFile"
                            ...
                            type="button" class="layui-btn btn-theme main_backgroundColor"
                            id="zuoyeShangchuanFileUpload">
                        <i v-if="true" :style='{"color":"#fff","show":true,"fontSize":"14px"}' class="layui-icon">&#xe67c;</i>上传作业上传
                    </button>
                </div>
            </div>
<%--            <div class="layui-form-item main_borderColor"--%>
...
<%--                    <input class="layui-input main_borderColor"--%>
<%--                           :style='{"padding":"0 12px","boxShadow":"0 0 0px rgba(160, 67, 26, 1)","backgroundColor":"#fff","color":"rgba(135, 206, 250, 1)","borderRadius":"4px","textAlign":"left","borderWidth":"1px","fontSize":"15px","borderStyle":"solid","height":"40px"}'--%>
<%--                           v-model="detail.zuoyeTijiaoPigai" lay-verify="integer|required" type="text"--%>
<%--                           :readonly="ro.zuoyeTijiaoPigai" name="zuoyeTijiaoPigai" id="zuoyeTijiaoPigai"--%>
<%--                           autocomplete="off">--%>
<%--                </div>--%>
<%--            </div>--%>
            <div class="layui-form-item main_borderColor"
...
<script>
    ...
    var vue = new Vue({
        ...
        data: {
            ...
            detail: {
                zuoyeTijiaoUuidNumber: new Date().getTime(),//数字
                zuoyeId: '',
                laoshiId: '',
                yonghuId: '',
                zuoyeTijiaoText: '',
    ...
    layui.use(['layer', 'element', 'carousel', 'http', 'jquery', 'form', 'upload', 'laydate', 'tinymce'], function () {
        ...
        localStorage.setItem("goUtl", "./pages/zuoyeTijiao/list.jsp")
        ...
        //var zuoyeId = http.getParam('zuoyeId');
        let zuoyeId = localStorage.getItem("zuoyeId")
        if (zuoyeId != null && zuoyeId != "") {
            //单个 作业查询
            initializationZuoye(zuoyeId);
        }
        //列表 作业查询
        initializationZuoyeList();
        // 下拉框
        // 提交状态的查询和初始化
        ...
        upload.render({
            //绑定元素
            elem: '#zuoyeShangchuanFileUpload',
            //上传接口
            ...
            // 上传成功
                ...
                if (res.code == 0) {
                    ...
                    jquery('#zuoyeShangchuanFile').val(url);
                    vue.detail.zuoyeShangchuanFile = url;
                    jquery('#zuoyeShangchuanFileImg').attr('src', url);
                } else {
                    layer.msg(res.msg, {
                        time: 2000,
                        icon: 5
        ...
        //修改
        if (http.getParam('zuoyeTijiaoId')) {
            http.request(`zuoyeTijiao/info/${http.getParam('zuoyeTijiaoId')}`, 'get', {}, function (data) {
        ...
        form.on('submit(thisSubmit)', function (data) {
            ...
            if (http.getParam('zuoyeTijiaoId')) {
                data["id"] = http.getParam('zuoyeTijiaoId')
            }
            data["zuoyeTijiaoYesnoTypes"] = 1;
            // 提交数据
            http.requestJson(`zuoyeTijiao/${http.getParam("zuoyeTijiaoId")?"update":"add"}`, 'post', data, function (res) {
                if (res.code == 0) {
                    layer.msg('提交成功', {
                        time: 2000,
                        icon: 6
                    }, function () {
                        vue.jump("../zuoyeTijiao/list.jsp");
                        http.remove('zuoyeTijiaoId')
                        // back();
                    });
                } else {
                    layer.msg(res.msg, {
    ...
    function zuoyeTijiaoYesnoTypesSelect() {
        //填充下拉框选项
        layui.http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=zuoye_tijiao_yesno_types", "GET", {}, (res) => {
            if (res.code == 0) {
    ...
    //单个 作业查询
    function initializationZuoye(id) {
        layui.http.request("zuoye/detail/" + id, 'get', {}, function (res) {
            if (res.code == 0) {
                vue.zuoye = res.data;
                vue.detail.zuoyeId = vue.zuoye.id//赋值给detail
            }
        });
    }

    //列表 作业查询
    function initializationZuoyeList() {
        layui.http.request("zuoye/page?page=1&limit=100&sort=&order=", 'get', {}, function (res) {
            if (res.code == 0) {
                vue.zuoyeList = res.data.list;
            }
        });
    }
</script>
