...
<!-- 标题 -->
 ...
 <div class="proleft fl" style="width: 20%;">
     ...
     <ul class="ul1">
         <li v-for="(item,index) in centerMenu" v-bind:key="index" :class="item.url=='../zuoye/list.jsp'?'on':''" @click="jump(item.url)">
             <b></b>{{item.name}}
         </li>
     </ul>
 </div>        <!-- 个人中心菜单 -->
            ...
            <div style="display: flex;height: 60px;align-items: center;margin-left: 15px;justify-content: space-between">
                <div style="display: flex;height: 60px;align-items: center;margin-left: 15px;">
                    <div class="thisTableType-search" style="margin-left: 15px;"
                         :style='searchForm.zuoyeTypes=="" || searchForm.zuoyeTypes==null?{"color":"red","borderBottom":"1px solid red"}:{"color":"#000"}'>
                        全部
                    </div>
                    <div style="margin-left: 15px;" v-for="item in zuoyeTypesList" :index="item.codeIndex" class="thisTableType-search"
                         :style='searchForm.zuoyeTypes==item.codeIndex?{"color":"red","borderBottom":"1px solid red"}:{"color":"#000"}'>
                        {{item.indexName}}
                    </div>
                </div>
<!--                <button @click="jump('../zuoye/add.jsp')" class="layui-btn layui-btn-lg btn-theme sub_backgroundColor" :style='{"padding":"0 15px","boxShadow":"0 0 8px rgba(0,0,0,0)","margin":"0 0 0 10px","borderColor":"#409EFF","color":"#333","borderRadius":"4px","borderWidth":"0","width":"auto","fontSize":"14px","borderStyle":"solid","height":"40px"}'>
                    <i v-if="true" class="layui-icon">&#xe654;</i> 添加作业信息
                </button>-->
            </div>
        ...
        <!-- 级联表 -->
                        ...
                        <td>{{item.zuoyeUuidNumber}}</td>
                        <td><img :src="baseUrl+item.zuoyePhoto" style="width: 100px;height: 100px;object-fit: cover;"></td>
                        <td>{{item.zuoyeValue}}</td>
                        <td v-if="item.zuoyeFile" @click="jump(baseUrl+item.zuoyeFile)" style="color: red">
                            作业下载下载
                        </td>
                        <td v-else>
                            无作业下载
                        </td>
                        <td><button v-if="item.zuoyeContent!=null&&item.zuoyeContent!=''" @click="showContentFunction(item.zuoyeContent)" type="button" class="layui-btn layui-btn-success">点击查看</button></td>
                        <td>{{item.insertTime}}</td>
                        <td style="width: 100px;">
                        <!--
                            <button @click="jump('../zuoye/add.jsp?zuoyeId='+item.id)" type="button" class="layui-btn layui-btn-sm layui-btn-radius layui-btn-warm">
...
<script type="text/javascript">
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage', 'http', 'jquery', 'laydate', 'tinymce'], function() {
        ...
        localStorage.setItem("goUtl","./pages/zuoye/list2.jsp")
