            ...
            <div style="display: flex;height: 60px;align-items: center;margin-left: 15px;justify-content: space-between">
                <div style="display: flex;height: 60px;align-items: center;margin-left: 15px;">
                    <div class="thisTableType-search" style="margin-left: 15px;"
                         :style='searchForm.yonghuTypes=="" || searchForm.yonghuTypes==null?{"color":"red","borderBottom":"1px solid red"}:{"color":"#000"}'>
                        全部
                    </div>
                    <div style="margin-left: 15px;" v-for="item in yonghuTypesList" :index="item.codeIndex" class="thisTableType-search"
                         :style='searchForm.yonghuTypes==item.codeIndex?{"color":"red","borderBottom":"1px solid red"}:{"color":"#000"}'>
                        {{item.indexName}}
                    </div>
                </div>
...
<script type="text/javascript">
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage', 'http', 'jquery', 'laydate', 'tinymce'], function() {
        ...
        $(document).on("click", ".thisTableType-search", function (e) {
            vue.searchForm.yonghuTypes = $(this).attr('index');
            pageList();
        });
           ...
           //当前表的 实训方向 字段 字典表查询方法
           function yonghuTypesSelect() {
               http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=yonghu_types", 'get', {}, function (res) {
                   if(res.code == 0){
