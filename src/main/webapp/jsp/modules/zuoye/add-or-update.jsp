                        ...
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 当前表的字段 -->
                                ...
                                <input id="laoshiId" name="laoshiId" type="hidden">
                                    ...
                                    <div class="form-group col-md-6 zuoyePhotoDiv">
                                        <label>作业封面</label>
                                        <img id="zuoyePhotoImg" src="" width="100" height="100">
                                        <input name="file" type="file" id="zuoyePhotoupload"
                                               class="form-control-file">
                                        <input name="zuoyePhoto" id="zuoyePhoto-input" type="hidden">
                                    </div>
                                    <div class="form-group col-md-6 zuoyeTypesDiv">
                                        <label>作业类型</label>
                                        <select style="width: 450px" id="zuoyeTypesSelect" name="zuoyeTypes" class="form-control"></select>
                                    </div>
...
<script>
    ...
    function upload() {
        ...
        <!-- 当前表的文件上传 -->
        $('#zuoyePhotoupload').fileupload({
            url: baseUrl + 'file/upload',
            headers: {token: window.sessionStorage.getItem("token")},
            dataType: 'json',
            done: function (e, data) {
                var photoUrl= 'file/download?fileName=' + data.result.file;
                document.getElementById('zuoyePhotoImg').setAttribute('src',baseUrl+photoUrl);
                document.getElementById('zuoyePhoto-input').setAttribute('value',photoUrl);
            }
        });
    ...
    <!-- 初始化当前表的下拉框 -->
        function initializationZuoyeTypesSelect(){
            ...
            if(zuoyeTypesSelect != null && zuoyeTypesOptions != null  && zuoyeTypesOptions.length > 0 ){
                zuoyeTypesSelect.options.length=0;
    ...
    function getContent() {
        ...
        if($("#zuoyeContentupload").length>0) {
            ...
            if (zuoyeContentEditor.hasContents()) {
                $('#zuoyeContent-input').attr('value', zuoyeContentEditor.getContent().replaceAll(baseUrl,""));
            }
        }
    }
    function jiaofei() {
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            ...
            messages: {
                laoshiId: "老师不能为空",
                zuoyeName: "作业名称不能为空",
                zuoyeUuidNumber: "作业编号不能为空",
                zuoyePhoto: "作业封面不能为空",
                zuoyeTypes: "作业类型不能为空",
                zuoyeFile: "作业下载不能为空",
                zuoyeContent: "作业介绍不能为空",
            }
        }).form();
    }
    ...
    function getDetails() {
        ...
        if (addzuoye != null && addzuoye != "" && addzuoye != "null") {
            ...
            $('#submitBtn').text('修改');
            if(window.sessionStorage.getItem('accountTableName') =='zuoye'){
