    ...
    <div id="content">
        ...
        <div class="container mt-0">
            <div class="row breadcrumb-bar">
                <div class="col-md-6">
                    <h3 class="block-title">学生考勤详情管理</h3>
                </div>
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">学生考勤详情列表</li>
                    </ol>
                </div>
            </div>
        </div>
            ...
            <div class="row">
                ...
                <div class="col-md-12">
                    <div class="widget-area-2 lochana-box-shadow">
                        <h3 class="widget-title">学生考勤详情列表</h3>
                        <div class="table-responsive mb-3" id="tableDiv">
                            <div class="col-sm-12">
                                ...
                                <label>考勤唯一编号</label>
                                <input type="text" id="yonghuKaoqinUuidNumberSearch" placeholder="请输入考勤唯一编号"
                                       aria-controls="tableId" class="form-control form-control-sm">

                                <label>考勤标题</label>
                                <input type="text" id="yonghuKaoqinNameSearch" placeholder="请输入考勤标题"
                                       aria-controls="tableId" class="form-control form-control-sm">
                                ...
                                <span class="导入导出">
                                            导入<input name="file" id="yonghuKaoqinListExcelFile" type="file"
                                                       class="btn btn-success"
                                                       style="width: 70px!important; line-height: 35px;">
                                            <button class="btn btn-success"
                                                    onclick="download('/upload/yonghuKaoqinListMuBan.xls')">导入模板下载</button>
                                        </span>
                            ...
                            <table id="tableId" class="table table-bordered table-striped">
                                ...
                                <tr>
                                    ...
                                    <th>考勤唯一编号<i id="yonghuKaoqinUuidNumberIcon"></i></th>
                                    <th>考勤标题<i id="yonghuKaoqinNameIcon"></i></th>
                                    <th onclick="sort('yonghu_kaoqin_list_types')">打卡状态<i
                                            id="yonghuKaoqinListTypesIcon" class="fa fa-sort"></i></th>
                                    <th onclick="sort('insert_time')">添加时间<i id="insertTimeIcon"
                                                                                 class="fa fa-sort"></i></th>
<%--                                    <th onclick="sort('update_time')">打卡时间<i id="updateTimeIcon"--%>
<%--                                                                                 class="fa fa-sort"></i></th>--%>
                                    <th>操作</th>
                                </tr>
                                </thead>
...
<script>
    ...
    function search() {
        ...
        //考勤唯一编号
        var yonghuKaoqinUuidNumberSearchInput = $('#yonghuKaoqinUuidNumberSearch');
        if (yonghuKaoqinUuidNumberSearchInput != null) {
            if (yonghuKaoqinUuidNumberSearchInput.val() != null && yonghuKaoqinUuidNumberSearchInput.val() != '') {
                searchForm.yonghuKaoqinUuidNumber = $('#yonghuKaoqinUuidNumberSearch').val();
            }
        }

        //考勤标题
        var yonghuKaoqinNameSearchInput = $('#yonghuKaoqinNameSearch');
        if (yonghuKaoqinNameSearchInput != null) {
            if (yonghuKaoqinNameSearchInput.val() != null && yonghuKaoqinNameSearchInput.val() != '') {
                searchForm.yonghuKaoqinName = $('#yonghuKaoqinNameSearch').val();
            }
        }
        <!-- 本表的查询条件 -->
    ...
    function getDataList() {
        http("yonghuKaoqinList/page", "GET", {
            page: pageIndex,
            limit: pageSize,
            sort: sortColumn,//字段
            order: sortOrder,//asc desc
            yonghuKaoqinListDelete: 1,
            //本表的
            //级联表的
            yonghuName: searchForm.yonghuName,

            yonghuKaoqinUuidNumber: searchForm.yonghuKaoqinUuidNumber,
            yonghuKaoqinName: searchForm.yonghuKaoqinName,
    ...
    function setDataRow(item, number) {
        ...
        //考勤唯一编号
        var yonghuKaoqinUuidNumberCell = document.createElement('td');
        yonghuKaoqinUuidNumberCell.innerHTML = item.yonghuKaoqinUuidNumber;
        row.appendChild(yonghuKaoqinUuidNumberCell);


        //考勤标题
        var yonghuKaoqinNameCell = document.createElement('td');
        yonghuKaoqinNameCell.innerHTML = item.yonghuKaoqinName;
        row.appendChild(yonghuKaoqinNameCell);


        //打卡状态
        var yonghuKaoqinListTypesCell = document.createElement('td');
        yonghuKaoqinListTypesCell.innerHTML = item.yonghuKaoqinListValue;
        row.appendChild(yonghuKaoqinListTypesCell);
        ...
        if (window.sessionStorage.getItem('role') == '学生' && item.yonghuKaoqinListTypes == 1) {
            //打卡
            var dakaBtn = document.createElement('button');
            var dakaAttr = "yonghuKaoqinListDaka(" + item.id + ')';
            dakaBtn.setAttribute("type", "button");
            dakaBtn.setAttribute("class", "btn btn-info btn-sm 打卡");
            dakaBtn.setAttribute("onclick", dakaAttr);
            dakaBtn.innerHTML = "打卡";
            btnGroup.appendChild(dakaBtn);
        }
    ...
    function upload() {
        /*导入数据*/
        $('#yonghuKaoqinListExcelFile').fileupload({
            url: baseUrl + 'file/upload',
            headers: {token: window.sessionStorage.getItem("token")},
            dataType: "json",
            done: function (e, data) {
                http("yonghuKaoqinList/batchInsert?fileName=" + data.result.file, "get", {}, (res) => {
                    if (res.code == 0) {
    ...
    function remove(id) {
        ...
        if (mymessage == true) {
            ...
            httpJson("yonghuKaoqinList/delete", "POST", paramArray, (res) => {
                if (res.code == 0) {
                    getDataList();
    ...
    function wuyong(id) {
        ...
        if (mymessage == true) {
            httpJson("yonghuKaoqinList/update", "POST", {
                id: id,
                yonghuKaoqinListTypes: 2
            }, (res) => {
                if (res.code == 0) {
                    getDataList();
                    layui.layer.msg("操作成功", {time: 2000, icon: 6});
    ...
    //添加
    function add() {
        window.sessionStorage.setItem("addyonghuKaoqinList", "addyonghuKaoqinList");
        window.location.href = "add-or-update.jsp"
    }
    ...
    function exportExcel() {
        excel = new ExcelGen({
            "src_id": "tableId",
            "show_header": true,
            "file_name": 'yonghuKaoqinList.xlsx'
        });
        excel.generate();
    }
    ...
    //打卡
    function yonghuKaoqinListDaka(id) {
        http("yonghuKaoqinList/info/" + id, "GET", {}, (res) => {
            if (res.data == null) {
                layui.layer.msg("查不到打卡信息,请核实", {time: 2000, icon: 5});
                return false;
            } else if (res.data.yonghuKaoqinListTypes == 2) {
                layui.layer.msg("已经打过卡了,不能重复打卡", {time: 2000, icon: 5});
                return false;
            } else {
                http("yonghuKaoqin/info/" + res.data.yonghuKaoqinId, "GET", {}, (res1) => {
                    if (res1.data == null) {
                        layui.layer.msg("被打卡表查询不到信息", {time: 2000, icon: 5});
                        return false;
                    } else {
                        var jiezhiTime = new Date(res1.data.jiezhiTime.replace(/-/g, "/"));
                        if (jiezhiTime.getTime() < new Date().getTime()) {
                            layui.layer.msg("超过打卡时间了,不能打卡了", {time: 2000, icon: 5});
                            return false;
                        }
                        res.data.yonghuKaoqinListTypes = 2;
                        //更新打卡状态并添加打卡时间
                        httpJson("yonghuKaoqinList/update", "POST", res.data, (res2) => {
                            if (res2.code == 0) {
                                layui.layer.msg("打卡成功", {time: 2000, icon: 6});
                                getDataList();
                            } else {
                                layui.layer.msg(res2.msg, {time: 2000, icon: 5});
                            }
                        });
                    }
                });
