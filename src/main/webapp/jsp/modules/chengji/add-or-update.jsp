                        ...
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 当前表的字段 -->
                                ...
                                <input id="laoshiId" name="laoshiId" type="hidden">
                                    <div class="form-group col-md-6 chengjiNameDiv">
                                        <label>成绩名称</label>
                                        <input style="width: 450px" id="chengjiName" name="chengjiName" class="form-control"
                                               placeholder="成绩名称">
                                    </div>
                                    <div class="form-group col-md-6 chengjiUuidNumberDiv">
                                        <label>成绩编号</label>
                                        <input style="width: 450px" id="chengjiUuidNumber" name="chengjiUuidNumber" class="form-control"
                                               placeholder="成绩编号">
                                    </div>
                                    <div class="form-group col-md-6 chengjiPhotoDiv">
                                        <label>成绩照片</label>
                                        <img id="chengjiPhotoImg" src="" width="100" height="100">
                                        <input name="file" type="file" id="chengjiPhotoupload"
                                               class="form-control-file">
                                        <input name="chengjiPhoto" id="chengjiPhoto-input" type="hidden">
                                    </div>
                                    <div class="form-group col-md-6 chengjiTypesDiv">
                                        <label>成绩类型</label>
                                        <select style="width: 450px" id="chengjiTypesSelect" name="chengjiTypes" class="form-control"></select>
                                    </div>
                                    <div class="form-group col-md-6 chengjiFenshuDiv">
                                        <label>分数</label>
                                        <input style="width: 450px" id="chengjiFenshu" name="chengjiFenshu" class="form-control"
                                               onchange="integerChickValue(this)"  placeholder="分数">
                                    </div>
...
<script>
    ...
    function upload() {
        ...
        <!-- 当前表的文件上传 -->
        $('#chengjiPhotoupload').fileupload({
            url: baseUrl + 'file/upload',
            headers: {token: window.sessionStorage.getItem("token")},
            dataType: 'json',
            done: function (e, data) {
                var photoUrl= 'file/download?fileName=' + data.result.file;
                document.getElementById('chengjiPhotoImg').setAttribute('src',baseUrl+photoUrl);
                document.getElementById('chengjiPhoto-input').setAttribute('value',photoUrl);
            }
        });
    ...
    <!-- 初始化当前表的下拉框 -->
        function initializationChengjiTypesSelect(){
            ...
            if(chengjiTypesSelect != null && chengjiTypesOptions != null  && chengjiTypesOptions.length > 0 ){
                chengjiTypesSelect.options.length=0;
    ...
    function getContent() {
        ...
        if($("#chengjiContentupload").length>0) {
            ...
            if (chengjiContentEditor.hasContents()) {
                $('#chengjiContent-input').attr('value', chengjiContentEditor.getContent().replaceAll(baseUrl,""));
            }
        }
    }
    function jiaofei() {
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                yonghuId: "required",
                laoshiId: "required",
                chengjiName: "required",
                chengjiUuidNumber: "required",
                chengjiPhoto: "required",
                chengjiTypes: "required",
                chengjiFenshu: "required",
                chengjiContent: "required",
            },
            messages: {
                yonghuId: "用户不能为空",
                laoshiId: "老师不能为空",
                chengjiName: "成绩名称不能为空",
                chengjiUuidNumber: "成绩编号不能为空",
                chengjiPhoto: "成绩照片不能为空",
                chengjiTypes: "成绩类型不能为空",
                chengjiFenshu: "分数不能为空",
    ...
    function getDetails() {
        ...
        if (addchengji != null && addchengji != "" && addchengji != "null") {
            //注册表单验证
            $(validform());
            $("#chengjiUuidNumber").val(new Date().getTime()+Math.ceil(Math.random()*10));//设置唯一号

            $('#submitBtn').text('新增');

        } else {
            $('#submitBtn').text('修改');
            if(window.sessionStorage.getItem('accountTableName') =='chengji'){
    ...
    <!--  当前表的数据回显  -->
        ...
        $("#chengjiName").val(ruleForm.chengjiName);
        $("#chengjiUuidNumber").val(ruleForm.chengjiUuidNumber);
        $("#chengjiFenshu").val(ruleForm.chengjiFenshu);
    ...
    function showImg() {
        <!--  当前表的图片  -->
        $("#chengjiPhotoImg").attr("src",baseUrl+ruleForm.chengjiPhoto);
