                    ...
                    <div class="widget-area-2 lochana-box-shadow">
                        ...
                        <div class="table-responsive mb-3" id="tableDiv">
                            <div class="col-sm-12">
                                ...
                                <label>成绩名称</label>
                                <input type="text" id="chengjiNameSearch" placeholder="请输入成绩名称"
                                       aria-controls="tableId" class="form-control form-control-sm">

                                <label>成绩类型</label>
                                <select name="chengjiTypesSelectSearch" id="chengjiTypesSelectSearch"
                                ...
                                <span class="导入导出">
                                            导入<input name="file" id="chengjiExcelFile" type="file"
                                                       class="btn btn-success"
                                                       style="width: 70px!important; line-height: 35px;">
                                            <button class="btn btn-success"
                                                    onclick="download('/upload/chengjiMuBan.xls')">导入模板下载</button>
                                        </span>
                            ...
                            <table id="tableId" class="table table-bordered table-striped">
                                ...
                                <tr>
                                    ...
                                    <th onclick="sort('chengji_name')">成绩名称<i id="chengjiNameIcon"
                                                                                  class="fa fa-sort"></i></th>
                                    <th onclick="sort('chengji_uuid_number')">成绩编号<i id="chengjiUuidNumberIcon"
                                                                                         class="fa fa-sort"></i></th>
                                    <th onclick="sort('chengji_photo')">成绩照片<i id="chengjiPhotoIcon"
                                                                                   class="fa fa-sort"></i></th>
                                    <th onclick="sort('chengji_types')">成绩类型<i id="chengjiTypesIcon"
                                                                                   class="fa fa-sort"></i></th>
                                    <th onclick="sort('chengji_fenshu')">分数<i id="chengjiFenshuIcon"
                                                                                class="fa fa-sort"></i></th>
                                    <th onclick="sort('insert_time')">录入时间<i id="insertTimeIcon"
                                                                                 class="fa fa-sort"></i></th>
                                    <th>操作</th>
                                </tr>
                                </thead>
...
<script>
    ...
    function search() {
        ...
        //成绩名称
        var chengjiNameSearchInput = $('#chengjiNameSearch');
        if (chengjiNameSearchInput != null) {
            if (chengjiNameSearchInput.val() != null && chengjiNameSearchInput.val() != '') {
                searchForm.chengjiName = $('#chengjiNameSearch').val();
            }
        }
    ...
    function getDataList() {
        http("chengji/page", "GET", {
            ...
            chengjiDelete: 1,
            //本表的
            chengjiName: searchForm.chengjiName,
    ...
    function setDataRow(item, number) {
        ...
        //成绩名称
        var chengjiNameCell = document.createElement('td');
        chengjiNameCell.innerHTML = item.chengjiName;
        row.appendChild(chengjiNameCell);


        //成绩编号
        var chengjiUuidNumberCell = document.createElement('td');
        chengjiUuidNumberCell.innerHTML = item.chengjiUuidNumber;
        row.appendChild(chengjiUuidNumberCell);

        //成绩照片
        var chengjiPhotoCell = document.createElement('td');
        var chengjiPhotoImg = document.createElement('img');
        var chengjiPhotoImgValue = baseUrl + item.chengjiPhoto;
        if (chengjiPhotoImgValue != null && chengjiPhotoImgValue != '' && chengjiPhotoImgValue != 'null') {
            chengjiPhotoImg.setAttribute('src', chengjiPhotoImgValue);
            chengjiPhotoImg.setAttribute('height', 100);
            chengjiPhotoImg.setAttribute('width', 100);
            chengjiPhotoCell.appendChild(chengjiPhotoImg);
        } else {
            chengjiPhotoCell.innerHTML = "暂无图片";
        }
        row.appendChild(chengjiPhotoCell);
        ...
        //分数
        var chengjiFenshuCell = document.createElement('td');
        chengjiFenshuCell.innerHTML = item.chengjiFenshu;
        row.appendChild(chengjiFenshuCell);
    ...
    function upload() {
        /*导入数据*/
        $('#chengjiExcelFile').fileupload({
            url: baseUrl + 'file/upload',
            headers: {token: window.sessionStorage.getItem("token")},
            dataType: "json",
            done: function (e, data) {
                http("chengji/batchInsert?fileName=" + data.result.file, "get", {}, (res) => {
                    if (res.code == 0) {
